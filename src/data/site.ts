...
export const site = {
  ...
  faq: [
    {
      q: '가입 신청이나 초대 코드가 필요한가요?',
      a: '아니요, 초대 코드나 별도의 신청·승인 절차 없이 누구나 byongari.com에서 바로 계정을 만들 수 있어요. 가입 후엔 프로필을 채우고 첫 글로 가볍게 인사해 주시면 돼요. 다만 지금은 가입이 열려 있지만, 이후 가입자 수에 따라 초대제로 바뀔 수 있어요. 서버 규칙에 동의하고 지켜주시는 것만 부탁드려요.',
    },
    {
      q: '마스토돈과 다른가요?',
      a: '체리픽은 마스토돈과 같은 페디버스(ActivityPub)라 서로 팔로우·소통할 수 있어요. 다만 마스토돈이 단순·텍스트 중심이라면, 체리픽은 미스키 기반이라 다양한 이모지 리액션, 글 꾸미기(MFM), 커스텀 이모지, 드라이브(파일 관리) 같은 기능이 풍부하고 UI도 더 아기자기해요.',
    },
    {
      q: '비공개 서버인가요?',
      a: '아니요, 폐쇄형 서버는 아니에요. 페디버스(연합)에 연결되어 있어서 마스토돈이나 다른 미스키·체리픽 서버의 사람들과도 서로 팔로우하고 소통할 수 있어요. 다만 작고 조용한 커뮤니티를 지향해요. 글마다 공개 범위(전체·홈·팔로워·다이렉트)를 직접 정할 수 있어서, 원하는 만큼만 공개하며 쓸 수 있어요.',
    },
    {
      q: '공개 범위가 뭔가요?',
      a: '글을 올릴 때 누구에게 보일지 고르는 설정이에요. 체리픽에는 보통 네 가지가 있어요 — 전체 공개(누구나 볼 수 있고 연합 타임라인까지), 홈(내 팔로워와 홈 타임라인에만), 팔로워(나를 팔로우한 사람에게만), 다이렉트(지정한 사람에게만, DM처럼). 여기에 ‘로컬만(연합 안 함)’ 옵션으로 byongari 서버 안에서만 보이게 할 수도 있어요. 글마다 자유롭게 바꿀 수 있어서 원하는 만큼만 공개하면 돼요.',
    },
  ],
